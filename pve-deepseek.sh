...
# 版本信息
CURRENT_VERSION="7.2.0-codex2"
BUILD_NICKNAME="Hana-Codex2"
...
sync_kernel_update() {
    ...
    local latest_kernel
    latest_kernel=$(apt-cache search --names-only '^(pve-kernel|proxmox-kernel)-[0-9].*-pve(-signed)?$' 2>/dev/null | awk '{print $1}' | sort -V | tail -1)
    if [[ -z "$latest_kernel" ]]; then
        log_warn "apt-cache 未找到内核包，尝试从镜像站获取..."
        get_available_kernels >/dev/null 2>&1
        latest_kernel=$(apt-cache search --names-only '^(pve-kernel|proxmox-kernel)-[0-9].*-pve(-signed)?$' 2>/dev/null | awk '{print $1}' | sort -V | tail -1)
    fi
...
enable_pass() {
    ...
    if grep -qi intel /proc/cpuinfo >/dev/null 2>&1; then
        iommu_param="intel_iommu=on"
    else
        iommu_param="amd_iommu=on"
    fi

    grub_add_param "$iommu_param"

    vfio_modules_content="vfio
vfio_iommu_type1
vfio_pci
vfio_virqfd
kvmgt"
    apply_block "/etc/modules" "IOMMU_VFIO" "$vfio_modules_content"
...
disable_pass() {
    ...
    grub_remove_param "$iommu_param"
    remove_block "/etc/modules" "IOMMU_VFIO"
...
# CPU/硬盘详细状态显示功能已迁移至 Codex2 实现（位于脚本末尾）
# 原旧版重复 cpu_add/cpu_del 已被移除
...
# 主程序
# ============ Codex2 hardware detail override ============
# Keep the full PVE-Tools feature set, but replace the node hardware-status
# patch with a PVE 9 oriented version that keeps the richer pve.sh style CPU
# and disk detail.
codex2_hw_install_packages() {
    local packages=(lm-sensors nvme-cli sysstat linux-cpupower hdparm smartmontools)
    local missing=()
    local package
    ...
    if (( ${#missing[@]} > 0 )); then
        log_step "安装硬件状态采集依赖: ${missing[*]}"
        apt-get update || return 1
        apt-get install -y "${missing[@]}" || return 1
    fi

    [[ -x /usr/sbin/linux-cpupower ]] && chmod +s /usr/sbin/linux-cpupower 2>/dev/null || true
    [[ -x /usr/sbin/nvme ]] && chmod +s /usr/sbin/nvme 2>/dev/null || true
    [[ -x /usr/sbin/smartctl ]] && chmod +s /usr/sbin/smartctl 2>/dev/null || true
    [[ -x /usr/sbin/hdparm ]] && chmod +s /usr/sbin/hdparm 2>/dev/null || true
    [[ -x /usr/sbin/turbostat ]] && chmod +s /usr/sbin/turbostat 2>/dev/null || true
    modprobe msr 2>/dev/null || true
    echo msr > /etc/modules-load.d/turbostat-msr.conf 2>/dev/null || true
}

codex2_hw_run_sensors_detect() {
    command -v sensors >/dev/null 2>&1 || return 0
    command -v sensors-detect >/dev/null 2>&1 || return 0

    if sensors -A 2>/dev/null | grep -Eqi 'coretemp|k10temp|zenpower|tctl|tdie|package id|core [0-9]'; then
        log_info "已检测到 CPU 传感器数据，跳过 sensors-detect"
        return 0
    fi

    local detect_choice drivers driver
    read -r -p "是否运行 sensors-detect --auto 补全 CPU/主板传感器？[Y/n]: " detect_choice
    if [[ "$detect_choice" =~ ^[Nn]$ ]]; then
        log_warn "已跳过 sensors-detect，若 Web UI 无温度数据可稍后重新执行本功能"
        return 0
    fi

    log_step "运行 sensors-detect --auto"
    sensors-detect --auto > /tmp/codex2-sensors-detect.log 2>&1 || {
        log_warn "sensors-detect 未完全成功，日志: /tmp/codex2-sensors-detect.log"
        return 0
    }

    drivers=$(sed -n '/Chip drivers/,/#----cut here/p' /tmp/codex2-sensors-detect.log 2>/dev/null | sed '/Chip /d;/cut/d' | xargs)
    for driver in $drivers; do
        modprobe "$driver" 2>/dev/null || true
        grep -qxF "$driver" /etc/modules 2>/dev/null || echo "$driver" >> /etc/modules
    done
    [[ -e /etc/init.d/kmod ]] && /etc/init.d/kmod start >/dev/null 2>&1 || true
}

codex2_hw_remove_markers() {
    local nodes="$1"
    local pvemanagerlib="$2"

    [[ -f "$nodes" ]] && perl -0pi -e 's/\n[ \t]*# codex2-hw-status-begin[\s\S]*?[ \t]*# codex2-hw-status-end\n/\n/g' "$nodes"
    [[ -f "$pvemanagerlib" ]] && perl -0pi -e 's/\n[ \t]*\/\/ codex2-hw-status-begin[\s\S]*?[ \t]*\/\/ codex2-hw-status-end\n/\n/g' "$pvemanagerlib"
}

cpu_add() {
    block_non_pve9_destructive "配置 Codex2 CPU/硬盘详细状态显示" || return 1
    ...
    local pvever tmpf ln disk_count core_count target_node_height target_cpu_height file

    for file in "$nodes" "$pvemanagerlib" "$proxmoxlib"; do
        if [[ ! -f "$file" ]]; then
            display_error "未找到 PVE Web UI 文件: $file" "请确认正在 PVE 9.x 宿主机上运行。"
            return 1
        fi
    done

    if grep -q "modbyshowtempfreq" "$nodes" "$pvemanagerlib" 2>/dev/null; then
        log_warn "检测到旧版 CPU/硬盘温度补丁。为避免重复插入，需要先恢复官方 Web UI 文件。"
        if confirm_action "重新安装官方 pve-manager / proxmox-widget-toolkit 后应用 Codex2 增强显示？"; then
            reinstall_pve_webui_packages || return 1
        else
            log_warn "已取消。请先在温度监控菜单中移除旧补丁，再重新执行。"
            return 1
        fi
    fi

    codex2_hw_install_packages || return 1
    codex2_hw_run_sensors_detect

    pvever=$(pveversion | awk -F"/" '{print $2}' | tr '/ ' '__')
    pvever="${pvever:-unknown}"
    [[ -f "${nodes}.${pvever}.codex2.bak" ]] || cp -a "$nodes" "${nodes}.${pvever}.codex2.bak"
    [[ -f "${pvemanagerlib}.${pvever}.codex2.bak" ]] || cp -a "$pvemanagerlib" "${pvemanagerlib}.${pvever}.codex2.bak"
    [[ -f "${proxmoxlib}.${pvever}.codex2.bak" ]] || cp -a "$proxmoxlib" "${proxmoxlib}.${pvever}.codex2.bak"

    codex2_hw_remove_markers "$nodes" "$pvemanagerlib"

    log_step "写入 Nodes.pm 硬件采集字段"
    tmpf=$(mktemp /tmp/pve-codex2-nodes.XXXXXX)
    cat > "$tmpf" <<'EOF'
        # codex2-hw-status-begin
        $res->{codex2_cpu_detail} = qx'
goverf=/sys/devices/system/cpu/cpufreq/policy0/scaling_governor
[ -f "$goverf" ] || goverf=/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor
...
[ -n "$cores" ] || cores=$(nproc 2>/dev/null || echo 0)
gov=$(cat "$goverf" 2>/dev/null || echo unknown)
min_khz=$(cat "$minf" 2>/dev/null || echo "")
max_khz=$(cat "$maxf" 2>/dev/null || echo "")
freqs=$(awk -F: "/cpu MHz/ {gsub(/^[ \\t]+/, \"\", \$2); if (\$2 != \"\") {if (out != \"\") out=out \",\"; out=out \$2}} END {print out}" /proc/cpuinfo 2>/dev/null)
pkgwatt=""
if command -v turbostat >/dev/null 2>&1 && command -v timeout >/dev/null 2>&1; then
    pkgwatt=$(timeout 3 turbostat --quiet --cpu package --show PkgWatt -S sleep 0.25 2>/dev/null | awk "NF && \$1 !~ /PkgWatt|-/ {v=\$1} END {print v}")
fi
printf "MODEL=%s\nCORES=%s\nGOV=%s\nMIN_KHZ=%s\nMAX_KHZ=%s\nFREQS_MHZ=%s\nPKGWATT=%s\n" "$model" "$cores" "$gov" "$min_khz" "$max_khz" "$freqs" "$pkgwatt"
        ';
        $res->{codex2_sensor_detail} = qx'
if command -v sensors >/dev/null 2>&1; then
    sensors -A 2>/dev/null
fi
        ';
        $res->{codex2_disk_detail} = qx'
for dev in /dev/nvme*n[0-9] /dev/sd[a-z] /dev/sd[a-z][a-z]; do
    [ -b "$dev" ] || continue
    name=${dev##*/}
    tran=$(lsblk -dn -o TRAN "$dev" 2>/dev/null | sed "s/^[[:space:]]*//;s/[[:space:]]*$//")
    rota=$(cat "/sys/block/$name/queue/rotational" 2>/dev/null || echo "")
    size=$(lsblk -dn -o SIZE "$dev" 2>/dev/null | sed "s/^[[:space:]]*//;s/[[:space:]]*$//")
    model=$(lsblk -dn -o MODEL "$dev" 2>/dev/null | sed "s/^[[:space:]]*//;s/[[:space:]]*$//")
    smart_info=$(smartctl -i "$dev" 2>/dev/null)
    if [ -z "$model" ]; then
        model=$(printf "%s\n" "$smart_info" | awk -F: "/Device Model|Model Number|Product/ {gsub(/^[ \\t]+/, \"\", \$2); print \$2; exit}")
    fi

    state=active
    if [ "$rota" = "1" ] && command -v hdparm >/dev/null 2>&1; then
        if ! printf "%s\n" "$smart_info" | grep -q "Transport protocol:.*SAS" && hdparm -C "$dev" 2>/dev/null | grep -iq "standby"; then
            state=standby
        fi
    fi
    ...
    if [ "$state" != "standby" ]; then
        if command -v timeout >/dev/null 2>&1; then
            smart_json=$(timeout 8 smartctl -a -j "$dev" 2>/dev/null)
        else
            smart_json=$(smartctl -a -j "$dev" 2>/dev/null)
        fi
        smart_b64=$(printf "%s" "$smart_json" | base64 2>/dev/null | tr -d "\n")
        if command -v iostat >/dev/null 2>&1; then
            io_raw=$(iostat -d -x -k 1 2 2>/dev/null | awk -v d="$name" "BEGIN{hdr=\"\"; line=\"\"} /^Device/ {hdr=\$0} \$1==d {line=\$0} END {if (line != \"\") {print hdr; print line}}")
            io_b64=$(printf "%s" "$io_raw" | base64 2>/dev/null | tr -d "\n")
        fi
    fi

    printf "BEGIN_DISK\nDEV=%s\nNAME=%s\nTRAN=%s\nROTA=%s\nSIZE=%s\nMODEL=%s\nSTATE=%s\nSMART_B64=%s\nIOSTAT_B64=%s\nEND_DISK\n" "$dev" "$name" "$tran" "$rota" "$size" "$model" "$state" "$smart_b64" "$io_b64"
done
        ';
        # codex2-hw-status-end
EOF

    ln=$(sed -n -e '/PVE::pvecfg::version_text/=' "$nodes" | head -n 1)
    if [[ -z "$ln" ]]; then
        rm -f "$tmpf"
        display_error "未找到 Nodes.pm 注入点" "没有匹配到 PVE::pvecfg::version_text，请检查 PVE 版本或手动恢复备份。"
        return 1
    fi
    ln=$((ln + 1))
    ...
    log_step "写入 pvemanagerlib.js 详细显示面板"
    tmpf=$(mktemp /tmp/pve-codex2-js.XXXXXX)
    cat > "$tmpf" <<'EOF'
// codex2-hw-status-begin
    {
        itemId: 'codex2CpuDetail',
        colspan: 2,
        printBar: false,
        title: gettext('CPU频率/功耗'),
        textField: 'codex2_cpu_detail',
        renderer: function(value) {
            function enc(v) {
                return Ext.String.htmlEncode(String(v === undefined || v === null ? '' : v));
            }
            function kvMap(text) {
                var out = {};
            ...
            function fmtGHz(mhz) {
                var n = Number(mhz);
                return Number.isFinite(n) ? (n / 1000).toFixed(2) + ' GHz' : '-';
            }
            function avg(arr) {
                return arr.length ? arr.reduce(function(a, b) { return a + b; }, 0) / arr.length : NaN;
            }
            function group(arr, n) {
                var rows = [];
                for (var i = 0; i < arr.length; i += n) rows.push(arr.slice(i, i + n).join(' | '));
                return rows;
            }

            var kv = kvMap(value);
            var freqs = (kv.FREQS_MHZ || '').split(',').map(Number).filter(Number.isFinite);
            if (!freqs.length && !kv.MODEL) return '无法获取 CPU 频率信息';

            var minNow = freqs.length ? Math.min.apply(null, freqs) : NaN;
            var maxNow = freqs.length ? Math.max.apply(null, freqs) : NaN;
            var avgNow = avg(freqs);
            var minCfg = kv.MIN_KHZ ? (Number(kv.MIN_KHZ) / 1000000).toFixed(2) + ' GHz' : '-';
            var maxCfg = kv.MAX_KHZ ? (Number(kv.MAX_KHZ) / 1000000).toFixed(2) + ' GHz' : '-';
            var watt = kv.PKGWATT ? ' | 功耗: <strong>' + enc(Number(kv.PKGWATT).toFixed(1)) + ' W</strong>' : '';
            var rows = [];

            rows.push('<strong>' + enc(kv.MODEL || 'CPU') + '</strong> | 线程: ' + enc(kv.CORES || freqs.length) + ' | 调速器: <strong>' + enc((kv.GOV || 'unknown').toUpperCase()) + '</strong>');
            if (freqs.length) {
                rows.push('实时: 平均 <strong>' + fmtGHz(avgNow) + '</strong> (' + fmtGHz(minNow) + '~' + fmtGHz(maxNow) + ') | 范围: ' + minCfg + '~' + maxCfg + watt);
                var coreRows = group(freqs.map(function(freq, idx) {
                    return '核心' + (idx + 1) + ': <strong>' + fmtGHz(freq) + '</strong>';
                }), 8);
                rows = rows.concat(coreRows);
            }
            return rows.join('<br>');
        }
    },
    {
        itemId: 'codex2SensorDetail',
        colspan: 2,
        printBar: false,
        title: gettext('CPU/主板温度'),
        textField: 'codex2_sensor_detail',
        renderer: function(value) {
            function enc(v) {
                return Ext.String.htmlEncode(String(v === undefined || v === null ? '' : v));
            }
            function colorTemp(temp) {
                var n = Number(temp);
                if (!Number.isFinite(n)) return enc(temp) + '°C';
                var color = n < 60 ? '#27ae60' : (n < 80 ? '#f39c12' : '#e74c3c');
                return '<span style="color:' + color + ';font-weight:600;">' + n.toFixed(0) + '°C</span>';
            }
            function group(arr, n) {
                var rows = [];
                for (var i = 0; i < arr.length; i += n) rows.push(arr.slice(i, i + n).join(' | '));
            ...
            if (!value || !String(value).trim()) return '未获取到传感器信息';
            var packages = [];
            var cores = [];
            var boards = [];
            var fans = [];
            ...
            String(value).split(/\n/).forEach(function(line) {
                var fan = line.match(/^\s*([^:]+):\s*([0-9]+)\s*RPM/i);
                if (fan) {
                    fans.push(enc(fan[1].trim()) + ': <strong>' + enc(fan[2]) + ' RPM</strong>');
                    return;
                }
                var m = line.match(/^\s*([^:]+):\s*\+?(-?[0-9.]+)\s*(?:°C|C)/i);
                if (!m) return;
                var label = m[1].trim();
                var item = enc(label) + ': ' + colorTemp(m[2]);
                if (/Core\s*\d+/i.test(label)) {
                    cores.push(item);
                } else if (/Package|Tctl|Tdie|Physical|CPU|CCD/i.test(label)) {
                    packages.push(item);
                } else if (/GFX|edge|junction|Composite/i.test(label)) {
                    boards.push('显卡/芯片组 ' + item);
                } else if (/temp|sys|systin|aux|mother|board|pch/i.test(label)) {
                    boards.push(item);
                }
            });

            var rows = [];
            if (packages.length) rows.push(packages.join(' | '));
            if (cores.length) rows = rows.concat(group(cores, 8));
            if (boards.length) rows.push(boards.join(' | '));
            if (fans.length) rows.push(fans.join(' | '));
            return rows.length ? rows.join('<br>') : '未识别到 CPU/主板温度传感器';
        }
    },
    {
        itemId: 'codex2DiskDetail',
        colspan: 2,
        printBar: false,
        title: gettext('硬盘详细状态'),
        textField: 'codex2_disk_detail',
        cellWrap: true,
        renderer: function(value) {
            function enc(v) {
                return Ext.String.htmlEncode(String(v === undefined || v === null ? '' : v));
            ...
            function decodeB64(text) {
                ...
                try {
                    ...
                    try { return atob(text); } catch (e2) { return ''; }
                }
            }
            ...
            function smartAttr(table, ids) {
                if (!Array.isArray(table)) return undefined;
                for (var i = 0; i < table.length; i++) {
                    if (ids.indexOf(table[i].id) !== -1 && table[i].raw) {
                        return table[i].raw.string !== undefined ? table[i].raw.string : table[i].raw.value;
                    }
                }
                return undefined;
            }
            function fmtBytes(bytes) {
                ...
                return n.toFixed(i < 3 ? 0 : 1) + units[i];
            }
            function fmtTemp(temp) {
                var n = Number(temp);
                if (!Number.isFinite(n)) return '';
                var color = n < 45 ? '#27ae60' : (n < 60 ? '#f39c12' : '#e74c3c');
                return '<span style="color:' + color + ';font-weight:600;">' + n.toFixed(0) + '°C</span>';
            }
            function fmtHealth(percent) {
                var n = Number(percent);
                if (!Number.isFinite(n)) return '';
                var color = n >= 80 ? '#27ae60' : (n >= 50 ? '#f39c12' : '#e74c3c');
                return '<span style="color:' + color + ';font-weight:600;">' + n.toFixed(0) + '%</span>';
            }
            function fmtNvmeUnits(units) {
                var n = Number(units);
                if (!Number.isFinite(n)) return '';
                return (n * 512000 / 1000000000000).toFixed(1) + 'T';
            }
            function parseIostat(b64) {
                var text = decodeB64(b64).trim();
                if (!text) return {};
                var lines = text.split(/\n/).filter(Boolean);
                if (lines.length < 2) return {};
                var headers = lines[0].trim().split(/\s+/).map(function(h) { return h.replace(/:$/, ''); });
                var values = lines[lines.length - 1].trim().split(/\s+/);
                var out = {};
                headers.forEach(function(h, idx) { out[h] = values[idx]; });
                return out;
            }
            function ioText(map) {
                if (!Object.keys(map).length) return '';
                var r = map['rkB/s'] || map['rKB/s'] || map['rMB/s'];
                var w = map['wkB/s'] || map['wKB/s'] || map['wMB/s'];
                var ra = map['r_await'];
                var wa = map['w_await'];
                var util = map['%util'];
                var parts = [];
                if (r || w) parts.push('I/O: 读 ' + enc(r || '-') + 'KB/s / 写 ' + enc(w || '-') + 'KB/s');
                if (ra || wa) parts.push('延迟: 读 ' + enc(ra || '-') + 'ms / 写 ' + enc(wa || '-') + 'ms');
                if (util) parts.push('负载: ' + enc(util) + '%');
                return parts.join(' | ');
            }

            if (!value || !String(value).trim()) return '未检测到可展示硬盘';
            var blocks = String(value).split('BEGIN_DISK\n').slice(1).map(function(block) {
                return block.split(/\nEND_DISK/)[0];
            });
            var rows = [];
            ...
            blocks.forEach(function(block) {
                var kv = kvMap(block);
                var dev = kv.DEV || kv.NAME || 'disk';
                var type = dev.indexOf('/dev/nvme') === 0 ? 'NVME' : (kv.ROTA === '1' ? 'HDD' : 'SSD');
                var smartText = decodeB64(kv.SMART_B64);
                var smart = {};
                try { smart = smartText ? JSON.parse(smartText) : {}; } catch (e) { smart = {}; }

                var model = smart.model_name || smart.model_number || get(smart, 'device.model_name') || kv.MODEL || 'unknown';
                var cap = fmtBytes(get(smart, 'user_capacity.bytes')) || kv.SIZE || '';
                var temp = get(smart, 'temperature.current') || smartAttr(get(smart, 'ata_smart_attributes.table'), [190, 194]);
                var hours = get(smart, 'power_on_time.hours') || smartAttr(get(smart, 'ata_smart_attributes.table'), [9]);
                var cycles = smart.power_cycle_count;
                var smartOk = get(smart, 'smart_status.passed');
                var nvmeLog = smart.nvme_smart_health_information_log || {};
                var used = nvmeLog.percentage_used;
                var health = used !== undefined ? Math.max(0, 100 - Number(used)) : undefined;
                var unsafe = nvmeLog.unsafe_shutdowns !== undefined ? nvmeLog.unsafe_shutdowns : smartAttr(get(smart, 'ata_smart_attributes.table'), [174, 192]);
                var mediaErrors = nvmeLog.media_errors;
                var io = ioText(parseIostat(kv.IOSTAT_B64));

                var parts = ['<strong>' + enc(type + ' ' + (kv.NAME || dev.replace('/dev/', ''))) + '</strong>', enc(model)];
                if (cap) parts.push('容量: ' + enc(cap));
                if (kv.STATE === 'standby') {
                    parts.push('<span style="color:#27ae60;font-weight:600;">休眠中</span>');
                    rows.push(parts.join(' | '));
                    return;
                }
                if (temp !== undefined && temp !== '') parts.push('温度: ' + fmtTemp(temp));
                if (health !== undefined && Number.isFinite(Number(health))) parts.push('寿命: ' + fmtHealth(health));
                if (smartOk !== undefined) parts.push('SMART: ' + (smartOk ? '<span style="color:#27ae60;">正常</span>' : '<span style="color:#e74c3c;font-weight:600;">警告</span>'));
                if (nvmeLog.data_units_read !== undefined || nvmeLog.data_units_written !== undefined) {
                    parts.push('读写: ' + (fmtNvmeUnits(nvmeLog.data_units_read) || '-') + ' / ' + (fmtNvmeUnits(nvmeLog.data_units_written) || '-'));
                }
                if (hours !== undefined && hours !== '') parts.push('通电: ' + enc(hours) + '小时' + (cycles !== undefined ? ', ' + enc(cycles) + '次' : ''));
                if (unsafe !== undefined && unsafe !== '') parts.push('异常断电: ' + enc(unsafe));
                if (mediaErrors !== undefined && Number(mediaErrors) > 0) parts.push('<span style="color:#e74c3c;font-weight:600;">介质错误: ' + enc(mediaErrors) + '</span>');
                if (io) parts.push(io);
                rows.push(parts.join(' | '));
            });

            return rows.length ? rows.join('<br>') : '未检测到可展示硬盘';
        }
    },
// codex2-hw-status-end
EOF

    ln=$(sed -n '/pveversion/,+10{/},/{=;q}}' "$pvemanagerlib" | head -n 1)
    if [[ -z "$ln" ]]; then
        rm -f "$tmpf"
        display_error "未找到 pvemanagerlib.js 注入点" "没有匹配到 pveversion 信息块，请检查 PVE Web UI 文件版本。"
        return 1
    fi
    sed -i "${ln}r $tmpf" "$pvemanagerlib"
    rm -f "$tmpf"

    disk_count=$(lsblk -dn -o TYPE 2>/dev/null | awk '$1=="disk"{c++} END{print c+0}')
    core_count=$(nproc --all 2>/dev/null || echo 1)
    target_node_height=$((520 + disk_count * 72 + ((core_count + 7) / 8) * 28))
    target_cpu_height=$((360 + ((core_count + 7) / 8) * 28))
    sed -i -r "/widget\.pveNodeStatus/,+8{/height:/{s#[0-9]+#${target_node_height}#}}" "$pvemanagerlib"
    sed -i -r "/widget\.pveCpuStatus/,+8{/height:/{s#[0-9]+#${target_cpu_height}#}}" "$pvemanagerlib"

    remove_subscription_popup
    systemctl restart pveproxy
    log_success "Codex2 CPU/硬盘详细状态显示已配置完成"
    log_info "请在浏览器中使用 Shift+F5 / Ctrl+F5 强制刷新缓存"
}
...
cpu_del() {
    ...
    log_warn "此操作会移除 Codex2 硬件状态显示补丁，并恢复 Web UI 文件"
    if ! confirm_action "恢复 CPU/硬盘详细状态显示补丁？"; then
        return 0
    fi

    if [[ -f "${nodes}.${pvever}.codex2.bak" && -f "${pvemanagerlib}.${pvever}.codex2.bak" && -f "${proxmoxlib}.${pvever}.codex2.bak" ]]; then
        cp -a "${nodes}.${pvever}.codex2.bak" "$nodes"
        cp -a "${pvemanagerlib}.${pvever}.codex2.bak" "$pvemanagerlib"
        cp -a "${proxmoxlib}.${pvever}.codex2.bak" "$proxmoxlib"
        systemctl restart pveproxy
        log_success "已从 Codex2 备份恢复 Web UI 文件"
        log_info "请在浏览器中使用 Shift+F5 / Ctrl+F5 强制刷新缓存"
        return 0
    fi

    log_warn "未找到当前 PVE 版本的 Codex2 备份，将尝试重装官方 Web UI 包"
    if reinstall_pve_webui_packages; then
        systemctl restart pveproxy
        log_success "已通过重装官方包移除硬件状态补丁"
    fi
}
