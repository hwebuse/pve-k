...
get_debian_ver() {
    local raw
    raw=$(awk -F"." '{print $1}' /etc/debian_version 2>/dev/null)
    case "$raw" in
...
pve_optimization() {
    ...
    echo " 1. 清华大学镜像站"
    echo " 2. 中科大镜像站"
    read -r -t 30 -p " 请选择 [默认1]: " aptsource
...
EOF
            ;;
        *) log_warn "无效选项，使用默认清华源"; aptsource=1
            cat > /etc/apt/sources.list <<-EOF
deb https://mirrors.tuna.tsinghua.edu.cn/debian/ ${sver} main contrib non-free non-free-firmware
deb https://mirrors.tuna.tsinghua.edu.cn/debian/ ${sver}-updates main contrib non-free non-free-firmware
deb https://mirrors.tuna.tsinghua.edu.cn/debian/ ${sver}-backports main contrib non-free non-free-firmware
...
EOF
    ...
    # CT模板源
    log_step "更换 CT 模板源..."
    if [[ -e /usr/share/perl5/PVE/APLInfo.pm ]]; then
        cp -rf /usr/share/perl5/PVE/APLInfo.pm /etc/apt/backup/APLInfo.pm.bak
        case "$aptsource" in
        ...
        log_warn "未找到 APLInfo.pm，跳过 CT 模板源更换"
    fi
...
EOF
    ...
    if [[ ! -f "$proxmoxlib" ]]; then
        log_warn "未找到 proxmoxlib.js，跳过去弹窗步骤"
    else
    ...
    if ! wget -q --timeout=10 --tries=2 "http://mirrors.tuna.tsinghua.edu.cn/proxmox/debian/proxmox-release-${sver}.gpg" -O "/etc/apt/trusted.gpg.d/proxmox-release-${sver}.gpg"; then
        log_warn "清华源下载密钥失败，尝试 GitHub 备用源..."
        if ! wget -q --timeout=10 --tries=2 "https://raw.githubusercontent.com/xiangfeidexiaohuo/pve-diy/master/gpg/proxmox-release-${sver}.gpg" -O "/etc/apt/trusted.gpg.d/proxmox-release-${sver}.gpg"; then
            log_warn "GitHub 备用源也失败，尝试从备份恢复..."
            [[ -e "/etc/apt/backup/proxmox-release-${sver}.gpg.bak" ]] && \
                cp "/etc/apt/backup/proxmox-release-${sver}.gpg.bak" "/etc/apt/trusted.gpg.d/proxmox-release-${sver}.gpg" && \
                log_warn "已从备份恢复原密钥，请手动下载新密钥" || \
                log_error "密钥下载失败且无备份，apt 可能无法正常工作！"
        fi
    fi
    log_success "密匙下载完成"
...
enable_pass() {
    log_step "开启硬件直通..."
    if [ $(dmesg | grep -ce 'DMAR\|IOMMU') -eq 0 ]; then
        log_error "您的硬件不支持直通！"
        return 1
    fi
    if [ $(grep -c Intel /proc/cpuinfo) -eq 0 ]; then
        iommu="amd_iommu=on"
    else
        iommu="intel_iommu=on"
    fi
    if [ $(grep -c "$iommu" /etc/default/grub) -eq 0 ]; then
        grub_add_param "$iommu"
        grub_add_param "iommu=pt"
        update-grub
        if [ $(grep -c "vfio_iommu_type1" /etc/modules) -eq 0 ]; then
            cat <<-EOF >> /etc/modules
vfio
vfio_iommu_type1
vfio_pci
...
cpupower_menu() {
    ...
    if [[ ! -f "$gov_file" ]]; then
        log_error "未找到 CPU 频率调节接口，当前环境可能不支持 cpufreq（如虚拟机或某些内核）"
        pause
        return 1
    fi
    governors=$(cat "$gov_file")
    while :; do
        clear
        echo -e "${H1}═════════════════════════════════════════════════${NC}"
...
cpu_add() {
    ...
    if [[ ${#missing_packages[@]} -gt 0 ]]; then
        log_info "以下包未安装，开始安装: ${missing_packages[*]}"
    ...
    [[ -e /usr/sbin/nvme ]] && chmod +s /usr/sbin/nvme
    [[ -e /usr/sbin/smartctl ]] && chmod +s /usr/sbin/smartctl
    ...
    if [ "$install" == "ok" ]; then
        ...
        if [ $(echo $drivers | wc -w) -eq 0 ]; then
            log_warn "没有找到任何驱动"
            pause
        else
            for i in $drivers; do
                modprobe "$i"
                if [ $(grep -c "^${i}$" /etc/modules) -eq 0 ]; then
                    echo "$i" >> /etc/modules
                fi
            done
            sensors
...
EOF
    ...
    if [[ -z "$ln" ]]; then
        log_error "未在 pvemanagerlib.js 中找到插入点，JS 注入失败"
        rm -f "$tmpf"
        return 1
    fi
...
pve9_ceph() {
    ...
    [[ -e /usr/share/perl5/PVE/CLI/pveceph.pm ]] && \
...
remove_ceph() {
    ...
    [[ "$confirm" != "y" ]] && return
    systemctl stop ceph-mon.target ceph-mgr.target ceph-mds.target ceph-osd.target 2>/dev/null || true
    rm -rf /etc/systemd/system/ceph*
    killall -9 ceph-mon ceph-mgr ceph-mds ceph-osd 2>/dev/null || true
    rm -rf /var/lib/ceph/mon/* /var/lib/ceph/mgr/* /var/lib/ceph/mds/* /var/lib/ceph/osd/*
    pveceph purge 2>/dev/null || true
    apt purge -y "ceph-mon" "ceph-osd" "ceph-mgr" "ceph-mds" "ceph-base" "ceph-mgr-modules-core" 2>/dev/null || true
...
remove_kernel() {
    ...
    available_kernels=$(dpkg --list 2>/dev/null | awk '$1 ~ /^(ii|hi)$/ && $2 ~ /^(pve|proxmox)-kernel-[0-9]/' | awk '{print $2}' | grep -v "$(uname -r)" | sort -V)
...
merge_local_storage() {
    ...
    log_info "正在扩展文件系统..."
    local fstype
    fstype=$(findmnt -n -o FSTYPE /dev/pve/root 2>/dev/null)
...
remove_swap() {
    ...
    log_info "正在扩展文件系统..."
    local fstype
    fstype=$(findmnt -n -o FSTYPE /dev/mapper/pve-root 2>/dev/null)
    [[ -z "$fstype" ]] && fstype="ext4"
    if [[ "$fstype" == "xfs" ]]; then
        xfs_growfs / 2>/dev/null || log_warn "xfs_growfs 失败，请手动执行 xfs_growfs /"
    else
...
pve_mail_setup() {
    ...
    if ! command -v postfix >/dev/null 2>&1; then
        ...
        # 确保依赖工具也已安装
        apt-get install -y mailutils libsasl2-modules >/dev/null 2>&1
    fi

    local from_addr root_addr
    read -r -p "请输入发件人邮箱: " from_addr
    [[ -z "$from_addr" ]] && { log_error "发件人邮箱不能为空"; return 1; }
    read -r -p "请输入 root 通知邮箱（收件人）: " root_addr
    ...
    case "$preset" in
        ...
        4)
            read -r -p "SMTP 服务器地址: " smtp_host
            read -r -p "SMTP 端口: " smtp_port
            read -r -p "TLS 模式（wrapper/starttls）[wrapper]: " tls_mode
    ...
    read -r -p "SMTP 登录账号 [${from_addr}]: " smtp_user
    ...
    # 测试发送
    read -r -p "是否发送测试邮件？(y/N): " test_choice
    ...
    while true; do
        ...
        case "$choice" in
            ...
            2)
                ...
                ;;
            3)
                read -r -p "请输入要安装的内核版本（如 proxmox-kernel-6.8.12-1-pve）: " kernel_ver
                if [[ -n "$kernel_ver" ]]; then
                    if apt-get install -y "$kernel_ver"; then
                        log_success "内核安装成功"
                        update-grub
                    else
                        log_error "内核安装失败"
                ...
                ;;
            4)
                read -r -p "请输入要设为默认的内核版本（如 6.8.12-1-pve）: " kernel_ver
                if [[ -n "$kernel_ver" ]]; then
            ...
            5)
                ...
                ;;
            6)
                read -r -p "确认重启系统？(y/N): " reboot_confirm
...
grub_add_param() {
    ...
    local current_line current_params param_key new_params
    ...
    new_params=$(echo "$current_params $param" | sed 's/  */ /g' | sed 's/^ //;s/ $//')
...
grub_remove_param() {
    ...
    current_params=$(echo "$current_line" | sed 's/^GRUB_CMDLINE_LINUX_DEFAULT="\(.*\)"$/\1/')
    param_key=$(echo "$param" | cut -d'=' -f1)
    new_params=$(echo "$current_params" | sed "s/\b${param_key}[^ ]*\b//g" | sed 's/  */ /g' | sed 's/^ //;s/ $//')
    sed -i "s|^GRUB_CMDLINE_LINUX_DEFAULT=.*|GRUB_CMDLINE_LINUX_DEFAULT=\"$new_params\"|" /etc/default/grub
...
igpu_sriov_setup() {
    ...
    read -r -p "请输入 i915-sriov-dkms 版本 [默认: ${dkms_ver}]: " input_ver
    ...
    if wget -q --show-progress -O /tmp/i915-sriov-dkms.deb "$deb_url"; then
        dpkg -i /tmp/i915-sriov-dkms.deb || log_error "驱动安装失败，请手动检查 dpkg 输出"
    else
        log_error "驱动下载失败，请检查网络或版本号是否正确"
    fi
...
rdm_list_disks() {
    ...
    while IFS= read -r link; do
        ...
        idx=$((idx + 1))
    done < <(find "$byid_dir" -maxdepth 1 -type l 2>/dev/null | sort)
    [[ $idx -eq 0 ]] && log_warn "未发现可直通磁盘"
}

rdm_attach() {
    rdm_list_disks
    read -r -p "请输入磁盘 by-id 路径（如 /dev/disk/by-id/ata-XXX）: " id_path
    [[ -z "$id_path" || ! -e "$id_path" ]] && { log_error "无效路径"; return 1; }
    read -r -p "请输入目标 VMID: " vmid
    [[ -z "$vmid" || ! "$vmid" =~ ^[0-9]+$ ]] && { log_error "无效 VMID"; return 1; }
    read -r -p "总线类型 (scsi/sata/ide) [scsi]: " bus
    bus=${bus:-scsi}
    # 查找可用插槽
    local slot=""
    local max_idx=30
...
rdm_detach() {
    ...
    read -r -p "请输入要移除的插槽（如 scsi1）: " slot
    ...
    while true; do
        ...
        case "$choice" in
            1)
                ...
                ;;
            2)
                read -r -p "请输入要添加的参数（如 intel_iommu=on）: " param
                if [[ -n "$param" ]]; then
                    grub_add_param "$param"
                    read -r -p "是否立即更新 GRUB？(Y/n): " upd
                    [[ "$upd" != "n" ]] && update-grub && log_success "GRUB 已更新"
                fi
                ;;
            3)
                read -r -p "请输入要删除的参数键名（如 intel_iommu）: " param
                if [[ -n "$param" ]]; then
