#!/bin/bash
# v2.3.0-safe 修复:
# - 修复 vfio 精确删除、cpufreq 缺失保护
# - 修复 IOMMU 数值比较、临时文件删除安全性
# - 强化 qm set 参数格式与 basename 引号
# - 改进 get_debian_ver 兼容性（去除无用 cat）
#
# v2.2.0-safe 修复:
# - 修复 blacklist.conf 逻辑，改为逐条检查追加
# - 初始化 install 变量，避免未定义使用
# - 修复 qm set 单横线参数问题（磁盘直通）
# - 全部 TUNA GPG 下载改为 HTTPS
# - 补充文件结尾换行
#
# 安全增强版说明:
# - 修复 basename 未引用、危险 rm 展开、HTTP GPG key 风险
# - 避免 cpupower_menu 中 local 兼容性问题
# - 版本号提升为 2.1.0-safe
# - 建议进一步人工审查 grub / kernel / ceph / JS patch 逻辑
#
# =============================================================================
# PVE-K 全能优化脚本 v2.0
...
EOF
    ...
    wget -q --timeout=5 --tries=1 https://mirrors.tuna.tsinghua.edu.cn/proxmox/debian/proxmox-release-${sver}.gpg -O /etc/apt/trusted.gpg.d/proxmox-release-${sver}.gpg
    if [[ $? -ne 0 ]]; then
...
EOF
        ...
        for mod in snd_hda_intel snd_hda_codec_hdmi i915; do
    grep -q "^blacklist ${mod}$" /etc/modprobe.d/blacklist.conf 2>/dev/null || echo "blacklist ${mod}" >> /etc/modprobe.d/blacklist.conf
done
...
disable_pass() {
    ...
    if [ $(grep -c "$iommu" /etc/default/grub) = 0 ]; then
        ...
        sed -i -E '/^(vfio|vfio_iommu_type1|vfio_pci|vfio_virqfd)$/d' /etc/modules
...
# ============ 3. CPU电源模式 ============
cpupower_menu() {
    governors=$([ -f /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors ] && cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors || echo "cpufreq_not_supported")
    while :; do
        clear
        echo -e "${H1}═════════════════════════════════════════════════${NC}"
        ...
        if [[ -n $(echo "$governors" | grep -o "$GOVERNOR") ]]; then
            ...
            cmd="sleep 10 && echo '$GOVERNOR' | tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor >/dev/null #CPU Power Mode"
